...
1. Navigate to the project root directory containing the `.csproj` file.
2. Execute the application:
   ```bash
   dotnet run
   ```
   or, for automatic rebuild on file changes:
   ```bash
   dotnet watch
   ```
3. Access the application via the localhost port provided in the terminal output.
